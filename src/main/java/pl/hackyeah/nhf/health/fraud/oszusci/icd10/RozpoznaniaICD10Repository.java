...
public class RozpoznaniaICD10Repository implements Repo {
    ...
    public List<ZagregowaneSrednieWarotsciGrupByICD10KodByRok> getTutek(String kodRozpoznania, int rok) {
        //@formatter:off
        return jdbcTemplate.query(
                "SELECT w.Rok, jgp_kod, avg(w.srednia_wartosc_grupy), liczba_wystapien FROM (\n" +
...
") as w where liczba_wystapien > 0.5 group by jgp_kod, w.Rok order by jgp_kod, Rok", (rs, rowNum)-> new pl.hackyeah.nhf.health.fraud.oszusci.icd10.domain.ZagregowaneSrednieWarotsciGrupByICD10KodByRok( rs.getInt(1),rs.getString(2), rs.getDouble(3), rs.getDouble(4)));
        //@formatter:on
    }
